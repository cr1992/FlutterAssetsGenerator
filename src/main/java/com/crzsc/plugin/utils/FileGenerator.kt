...
//            showNotify("assets path is empty")
        ...
        var leadingText = ""
        if (FileHelperNew.isWithLeadingWithPackageName(config)) {
            leadingText = "packages/${config.map["name"]}/"
        }
        map.toSortedMap().forEach {
            content.append("  static const String ${it.key} = '${leadingText}${it.value}';\n")
        }
        content.append("\n}\n")
