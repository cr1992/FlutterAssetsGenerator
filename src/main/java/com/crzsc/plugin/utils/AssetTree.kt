...
enum class MediaType {
    IMAGE, SVG, LOTTIE, UNKNOWN, DIRECTORY
}
...
object AssetTreeBuilder {
     ...
     */
    fun build(files: List<VirtualFile>, moduleRootPath: String, ignorePath: List<String>): AssetNode {
        LOG.info("[FlutterAssetsGenerator #AssetTreeBuilder] [AssetTreeBuilder] Building asset tree from ${files.size} files")
        // 创建根节点
        val rootNode = AssetNode("Assets", "", MediaType.DIRECTORY, null)
        
        for (file in files) {
            LOG.info("[FlutterAssetsGenerator #AssetTreeBuilder] [AssetTreeBuilder] Processing file: ${file.path}")
    ...
    private fun processFile(
        ...
        LOG.info("[FlutterAssetsGenerator #AssetTreeBuilder] [processFile] Processing: ${file.path}, isDirectory: ${file.isDirectory}")

        if (file.isDirectory) {
            // 如果是目录,为该目录创建节点(如果不存在)
            val relativePath = file.path.removePrefix("$moduleRootPath/")
            LOG.info("[FlutterAssetsGenerator #AssetTreeBuilder] [processFile] Directory relativePath: $relativePath")
            
            // 查找或创建当前目录节点
            val dirName = file.name
            var dirNode = parentNode.children.find { it.name == dirName && it.type == MediaType.DIRECTORY }
            if (dirNode == null) {
                LOG.info("[FlutterAssetsGenerator #AssetTreeBuilder] [processFile] Creating directory node: $dirName (path: $relativePath)")
                dirNode = AssetNode(dirName, relativePath, MediaType.DIRECTORY, null)
                parentNode.children.add(dirNode)
            } else {
                LOG.info("[FlutterAssetsGenerator #AssetTreeBuilder] [processFile] Found existing directory node: $dirName")
            }
            ...
            LOG.info("[FlutterAssetsGenerator #AssetTreeBuilder] [processFile] Adding file: $name (path: $relativePath, type: $mediaType)")
    ...
    /**
     * 根据目录路径查找对应的节点，如果不存在则创建
     * 这是为了处理 pubspec 可能配置了深层目录的情况
     */
    private fun getNodeForPath(
        ...
        LOG.info("[FlutterAssetsGenerator #AssetTreeBuilder] [getNodeForPath] directory: ${directory.path}, relativePath: $relativePath")
        ...
        for (part in parts) {
            ...
            var child = currentNode.children.find { it.name == part && it.type == MediaType.DIRECTORY }
            if (child == null) {
                // 不存在则创建,使用完整的相对路径
                LOG.info("[FlutterAssetsGenerator #AssetTreeBuilder] [getNodeForPath] Creating new directory node: $part (path: $currentPath)")
                child = AssetNode(part, currentPath, MediaType.DIRECTORY, null)
                currentNode.children.add(child)
            } else {
                LOG.info("[FlutterAssetsGenerator #AssetTreeBuilder] [getNodeForPath] Found existing directory node: $part")
            }
            currentNode = child
        }
        return currentNode
    ...
    private fun shouldIgnore(file: VirtualFile, ignorePath: List<String>): Boolean {
        ...
        // 忽略配置列表中的文件
        return ignorePath.any { file.path.contains(it, ignoreCase = true) }
    }
    ...
    /**
     * 简单判断是否为 Lottie 动画文件
     * 检查 JSON 中是否包含 "v" (版本) 和 "layers" 或 "ip" 等关键字
     */
    private fun isLottieFile(file: VirtualFile): Boolean {
        return try {
            if (file.length > 5 * 1024 * 1024) return false // 跳过大于 5MB 的文件
